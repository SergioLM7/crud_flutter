...
class HouseService {
  ...
  Future<List<House>> getHouses() async {
    final response = await _supabaseDB.from('houses').select();
  ...
  Future<void> addHouse(String name, String founder) async {
    await _supabaseDB.from('houses').insert({'name': name, 'founder': founder});
  }

  Future<void> updateHouse(String id, String name, String founder) async {
    await _supabaseDB.from('houses').update({'name': name, 'founder': founder}).eq('id', id);
  }

  Future<void> deleteHouse(String id) async {
    await _supabaseDB.from('houses').delete().eq('id', id);
  }
}