...
class WizardService {
  ...
  Future<void> addWizard(String name, int age, String? houseId, String? wandId) async {
    await _supabaseDB.from('wizard').insert({'name': name, 'age': age, 'houseId': houseId, 'wandId': wandId});
  }

  Future<void> updateWizard(String id, String name, int age, String? houseId, String? wandId) async {
    await _supabaseDB.from('wizard').update({'name': name, 'age': age, 'houseId': houseId, 'wandId': wandId}).eq('id', id);
  }
